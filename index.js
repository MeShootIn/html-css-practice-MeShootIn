 ...
 * da979bab
 */
import OmdbApi from './src/api/omdb-api.js';
...
qSelector('.search-form__tags')
    .appendChild($searchTagContainer);
qSelector('.search-form')
...
async function runSearch(title) {
    ...
    const $resultsResume = qSelector('.results__resume');
    try {
        const $resultContainer = qSelector('.result-container');
        $resultContainer.innerHTML = '';
        const result = await bySearchAll(api, title, window.navigator.hardwareConcurrency);
        ...
        for (const promise of result.BySearchPromises) {
            promise.then(bySearchResponse => {
                ...
                bySearchResponse.Search.forEach(async (shortInfo) => {
                    ...
                    $resultContainer.appendChild(new FilmCard(byIdResponse.Title, byIdResponse.Year, byIdResponse.Poster, byIdResponse.Ratings, byIdResponse.Genre, `https://www.imdb.com/title/${shortInfo.imdbID}/`));
                });
            });
        }
    }
